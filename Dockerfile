...
RUN apt-get update \
    && apt-get -y upgrade \
    && apt-get -y -f install locales vim ssh python3 cmake libengine-pkcs11-openssl pesign libp11-kit-dev libnss3-tools libjson-c-dev libnss3-dev libssl-dev libcurl4-openssl-dev bash-completion sudo curl screen diffstat gawk chrpath wget cpio texinfo less mc perl libtext-unidecode-perl libxml-libxml-perl pigz \
    && apt-get install -f \
